...
uint8_t crc8(uint8_t *message, uint8_t message_size) {
  uint8_t rem = 0;
  for (uint8_t i = 0; i < message_size; ++i) {
    rem = crc8_table[rem ^ message[i]];
  }
  return rem;
}

// Uses only 16 bytes instead of 256, estimated to be ~4 times slower
uint8_t crc8_4bit(uint8_t *message, uint8_t message_size) {
  uint8_t rem = 0;
  for (uint8_t i = 0; i < message_size; ++i) {
    uint8_t msg = message[i];
    rem = crc8_table[(rem ^ msg) >> 4] ^ (rem << 4);
    rem = crc8_table[(rem >> 4) ^ (msg & 0x0F)] ^ (rem << 4);
  }
  return rem;
}
...
uint8_t crc4_4bit(uint8_t *message, uint8_t message_size) {
  uint8_t rem = 0;
  for (uint8_t i = 0; i < message_size; ++i) {
    uint8_t msg = message[i];
    rem = crc4_table[(msg >> 4) ^ rem];
    rem = crc4_table[(msg & 0x0F) ^ rem];
  }
  return rem;
}
